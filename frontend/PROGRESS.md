...
Automata is a **cross-chain automation platform built on Polkadot**. Think of it as Zapier for blockchains.
...
**Why it matters:** Today, executing cross-chain actions requires deep technical knowledge and multiple manual steps. Automata collapses that into a visual, one-click experience.
...
├── contracts/
│   ├── core/
│   │   ├── AutomataCore.sol         ← main contract
...
│   └── abi/
│       └── AutomataCore.json        ← contract ABI (copied from contracts/)
│
...
| Flow builder library    | React Flow                     | Purpose-built for node-based UIs, perfect fit for Automata        | Project start |
...
| AutomataCore   | Moonbase Alpha | NOT YET DEPLOYED | —           |
...
- [ ] Delete the default Next.js boilerplate from `app/page.tsx` — replace with `<h1>Automata</h1>`
- [ ] Run `npm run dev` — loads at localhost:3000 with no errors

```
DONE WHEN: localhost:3000 shows "Automata" heading. Zero console errors.
```
...
### Step 2.4 — AutomataCore.sol

- [ ] Create `contracts/core/AutomataCore.sol`
...
- [ ] Write `test/AutomataCore.test.ts` — test execute(), test replay protection, test empty payload revert, test recipe execution
...
- [ ] Create `scripts/deploy.ts` — deploys RecipeRegistry first, then AutomataCore with registry address
...
- [ ] Create `app/layout.tsx`:
  ...
  - Top nav: Automata logo left, nav links (Build, Recipes, History) center, wallet button right
...
- [ ] Import ABI from `src/abi/AutomataCore.json`
...
- [ ] On every successful execution: save to localStorage `Automata_history` array:
  ```json
  {
    "flowId": "0x...",
    "txHash": "0x...",
...
```
DONE WHEN: MetaMask shows AutomataCore contract address in the transaction popup.
```
...
_This file is the project. Keep it updated. Every session starts here._
